...
const labelPresenter = ({datum}: {datum: Externals.Github.ContributionDay}) => `Date: ${moment(new Date(datum.date)).format('dd/MM/yyyy')} \nContributions: ${datum.contributionCount}\n`

export const VictoryGithubChart: FC<Props> = ({ contributionDays }) => {
  ...
  return (
    <div>
      ...
      >
        <VictoryAxis
          tickFormat={(x) => {
            const formatted = moment(x).format('yyyy-MM-dd')
            return formatted
          }}
        />
        <VictoryLine
